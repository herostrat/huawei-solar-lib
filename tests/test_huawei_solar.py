...
async def test_get_model_name(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_multiple(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_state_1(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_state_2(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_state_3(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_alarm_1_some(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_alarm_2_some(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_alarm_3_some(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.PV_01_VOLTAGE)
    assert result.value == 0.0
    assert result.unit == "V"


async def test_get_input_power(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.INPUT_POWER)
    assert result.value == 0
    assert result.unit == "W"


async def test_get_grid_voltage(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.GRID_VOLTAGE)
    assert result.value == 0.0
    assert result.unit == "V"


async def test_get_line_voltage_a_b(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.LINE_VOLTAGE_A_B)
    assert result.value == 0
    assert result.unit == "V"


async def test_get_line_voltage_b_c(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.LINE_VOLTAGE_B_C)
    assert result.value == 0
    assert result.unit == "V"


async def test_get_line_voltage_c_a(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.LINE_VOLTAGE_C_A)
    assert result.value == 0
    assert result.unit == "V"


async def test_get_line_phase_a_voltage(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.PHASE_A_VOLTAGE)
    assert result.value == 0
    assert result.unit == "V"


async def test_get_line_phase_b_voltage(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.PHASE_B_VOLTAGE)
    assert result.value == 0
    assert result.unit == "V"


async def test_get_line_phase_c_voltage(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_insulation_resistance(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.INSULATION_RESISTANCE)
    assert result.value == 3.0
    assert result.unit == "MOhm"


async def test_get_device_status(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.DEVICE_STATUS)
    assert result.value == "Standby: no irradiation"
    assert result.unit is None


async def test_get_device_status_invalid(huawei_solar: AsyncHuaweiSolarClient):
    with (
        patch.object(
            huawei_solar,
            "execute",
...
async def test_get_fault_code(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.FAULT_CODE)
    assert result.value == 0
    assert result.unit is None


async def test_get_accumulated_yield_energy(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.ACCUMULATED_YIELD_ENERGY)
    assert result.value == 207.34
    assert result.unit == "kWh"


async def test_get_daily_yield_energy(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.DAILY_YIELD_ENERGY)
    assert result.value == 0.65
    assert result.unit == "kWh"


async def test_get_nb_optimizers(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.NB_OPTIMIZERS)
    assert result.value == 10
    assert result.unit is None


async def test_get_nb_online_optimizers(huawei_solar: AsyncHuaweiSolarClient):
    result = await huawei_solar.get(rn.NB_ONLINE_OPTIMIZERS)
    assert result.value == 0
    assert result.unit is None


async def test_get_grid_code(huawei_solar: AsyncHuaweiSolarClient):
...
async def test_get_time_zone(huawei_solar: AsyncHuaweiSolarClient):
